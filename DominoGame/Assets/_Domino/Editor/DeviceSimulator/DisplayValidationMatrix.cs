...
namespace Domino.Editor
{
    ...
    public sealed class DisplayValidationMatrix : EditorWindow
    {
        ...
        void OnGUI()
        {
            EditorGUILayout.HelpBox("Selecciona el perfil desde Device Simulator y prueba Landscape Left/Right. iPhone: safe area documentada, sin silueta exacta de Dynamic Island. Android Synthetic: escenarios de prueba, no modelos físicos.", MessageType.Info);
    ...
    public sealed class DominoDisplayValidationPlugin : DeviceSimulatorPlugin
    {
        ...
        static string ValidateCurrent()
        {
            ...
            if (!Application.isPlaying || !controller || controller.View.IsPreparingRound) return "Pendiente: entra en Play y espera el reparto.";
            int failures = 0;
