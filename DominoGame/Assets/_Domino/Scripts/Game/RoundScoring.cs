...
namespace Domino.Game
{
    ...
    public static class RoundScoring
    {
        ...
        {
            if (points == null || points.Length != 4 || Array.Exists(points, p => p < 0) || multiplier < 1)
                throw new ArgumentException("Four nonnegative hand totals and a positive multiplier are required.");
            int winner = finisher;
            if (blocked)
            {
                ...
                {
                    int value = points[p];
                    if (rules.Teams && rules.BlockedWinner == BlockedWinnerRule.LowestTeamTotal) value += points[(p + 2) % 4];
            ...
            if (winner < 0 || winner > 3) throw new ArgumentOutOfRangeException(nameof(finisher));
            int total = 0;
            for (int p = 0; p < 4; p++)
                if (rules.Scoring == PointsSource.AllOtherPlayers ? p != winner : rules.Side(p) != rules.Side(winner)) total += points[p];
            return new RoundResult(winner, rules.Side(winner), blocked, total, blocked ? 0 : rules.FinishBonus, multiplier, points);
        }
    }
    ...
    public sealed class MatchState
    {
        ...
        public MatchState(GameRules rules) { this.rules = rules; }
        ...
        public void Apply(RoundResult result)
        {
            ...
            if (result.Tie) Multiplier = rules.CompoundTies ? checked(Multiplier * 2) : 2;
            else
            {
