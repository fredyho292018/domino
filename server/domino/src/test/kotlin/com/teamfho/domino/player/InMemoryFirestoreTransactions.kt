...
internal class InMemoryFirestoreTransactions(val now: Instant) {
    ...
    init {
        ...
        `when`(firestore.runTransaction(any<Transaction.Function<BootstrapResult>>(), any(TransactionOptions::class.java)))
            .thenAnswer { invocation ->
                synchronized(lock) {
                    try {
                        injectedFailure?.let { throw it }
                        val function = invocation.getArgument<Transaction.Function<BootstrapResult>>(0)
                    ...
                    } catch (error: Throwable) { ApiFutures.immediateFailedFuture<BootstrapResult>(error) }
                }
            }
    }
}
