...
namespace Domino.UI
{
    ...
    public static class BoardLayout
    {
        ...
        public static TilePose[] Arrange(int count, System.Func<int, bool> isDouble)
            => ArrangeInArea(count, isDouble, new Vector2(920, 360), 650);

        public static TilePose[] Arrange(int count, System.Func<int, bool> isDouble, Vector2 availableSize)
        {
            // Build along the longest axis, leaving room for a complete corner and doubles.
            bool portrait = availableSize.y > availableSize.x;
            const float readableScale = 1.7f;
            var area = (portrait ? new Vector2(availableSize.y, availableSize.x) : availableSize) / readableScale;
            var result = ArrangeInArea(count, isDouble, area, Mathf.Max(160, area.x - 150));
            for (int i = 0; i < result.Length; i++)
            {
                var p = result[i];
                var position = portrait ? new Vector2(-p.Position.y, p.Position.x) : p.Position;
                result[i] = new TilePose(position.x * readableScale, position.y * readableScale,
                    (p.Angle + (portrait ? 90 : 0)) % 360, p.Scale * readableScale);
            }
            return result;
        }
        static TilePose[] ArrangeInArea(int count, System.Func<int, bool> isDouble, Vector2 area, float runLength)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                else
                {
                    ...
                    else if (rowWidth + TileGap + half.x * 2 <= runLength
                        || crosswise
            ...
            float fit = Mathf.Min(1, Mathf.Min(area.x / (max.x - min.x), area.y / (max.y - min.y)));
            for (int i = 0; i < count; i++)
            {
