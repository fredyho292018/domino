...
namespace Domino.Client
{
    ...
    public sealed class SessionSetup
    {
        ...
        public bool SharedDevice => !Rules.Mode.BotsAllowed;
        ...
        public SessionSetup(MatchRuleSnapshot rules, int localPlayerSeat = 0)
        {
            ...
            int humans=rules.Mode.BotsAllowed?1:Configuration.PlayerCount;
            if(humans<rules.Mode.MinHumans||humans>rules.Mode.MaxHumans)throw new ArgumentException("Local session needs one human and enabled bots.");
            var controls=new ParticipantControl[Configuration.PlayerCount];
            for(int seat=0;seat<controls.Length;seat++)controls[seat]=rules.Mode.BotsAllowed?ParticipantControl.BOT:ParticipantControl.LOCAL_HUMAN;
