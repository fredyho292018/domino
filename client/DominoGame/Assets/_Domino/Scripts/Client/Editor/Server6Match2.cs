...
namespace Domino.Editor {
 public static class Server6Match2 {
  const BindingFlags Fields=BindingFlags.Instance|BindingFlags.NonPublic;
  static string Dir=>Path.GetFullPath(Path.Combine(Application.dataPath,"../../Validation/Generated/SERVER6-match2"));
  ...
  [MenuItem("Domino/Validation/SERVER-6/Execute MATCH 2 only")]
  public static async void Run(){
   ...
   running=true;errors=0;started=completed=1;Application.logMessageReceived+=Log;string stage="START";
   try{
    ...
    Check(((JArray)before["items"]).Count==1&&(string)before["items"][0]["history"]["matchId"]==firstId,"INITIAL_HISTORY");
    ...
    Check(((JArray)final["items"]).Count==2&&final["nextCursor"]?.Type!=JTokenType.String,"FINAL_HISTORY_COUNT");
    Check((string)final["items"][1]["history"]["matchId"]==firstId,"MATCH_1_MISSING_OR_ORDER");
    string secondId=File.ReadAllText(Path.Combine(Dir,"match-2.txt")).Trim();
    Check((string)final["items"][0]["history"]["matchId"]==secondId,"MATCH_2_ORDER");
    var firstManifest=await new ReplayClient(ApplicationServices.OnlineApi).Manifest(firstId,CancellationToken.None);
    Check((bool?)firstManifest["replayAvailable"]==true,"MATCH_1_REPLAY_UNAVAILABLE");
    ...
    Check(Field<System.Collections.Generic.List<JObject>>(hv,"history").Count==2,"HISTORY_UI_COUNT");
    Record("UNITY_HISTORY_ENTRIES_VISIBLE_AFTER_MATCH_2=2");ScreenCapture.CaptureScreenshot(Path.Combine(Dir,"history.png"));await Task.Delay(1000);
    ...
    Guard();Record("MATCH_2_GATE=PASS\nMATCH_3_STARTED=NO");Debug.Log("[SERVER6-MATCH2] Match 2 PASS; stopped before Match 3.");
