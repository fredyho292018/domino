...
namespace Domino.Editor {
 public static class Server6FiveMatches {
  const BindingFlags Fields=BindingFlags.Instance|BindingFlags.NonPublic;
  static string Dir=>Path.GetFullPath(Path.Combine(Application.dataPath,"../../Validation/Generated/SERVER6-match1"));
  ...
  static void Record(string text)=>File.AppendAllText(Path.Combine(Dir,"execution.txt"),text+"\n");
  ...
  [MenuItem("Domino/Validation/SERVER-6/Execute MATCH 1 only")]
  public static async void Run(){
   ...
   running=true;errors=started=completed=0;Application.logMessageReceived+=Log;string stage="START";
   try{
    ...
    for(int n=1;n<=1;n++){
     ...
     await Until(()=>File.Exists(Path.Combine(Dir,"participants-confirmed-"+n)),"PARTICIPANT_CORRELATION_TIMEOUT",45);
     Check(File.ReadAllText(Path.Combine(Dir,"participants-confirmed-"+n)).Trim()==id,"PARTICIPANT_CORRELATION_FAILED");
     Record("DISTINCT_MATCH_PLAYERS=4\nCURRENT_UNITY_USER_IN_MATCH_1=YES");string rejected=null;client.Rejected+=c=>rejected=c;long last=client.Snapshot.Sequence;
     ...
     var proof=JObject.Parse(File.ReadAllText(Path.Combine(Application.dataPath,"../../Validation/Generated/S606R/identity-seats.json")));
     ...
     Server6ParticipantValidator.Validate(authority,historyItem,manifest,Server6ParticipantValidator.Match1Expected,identities,client.Snapshot.Seat);
     Record("MATCH_1_HISTORY_CONSISTENCY=PASS\nMATCH_1_RESULT="+manifest["result"]["winner"].ToString(Newtonsoft.Json.Formatting.None)+"\nMATCH_1_SCORE="+manifest["finalScore"].ToString(Newtonsoft.Json.Formatting.None)+"\nMATCH_1_COMPLETION_REASON=TARGET_REACHED");
     ...
     UnityEngine.Object.Destroy(view.gameObject);await Task.Delay(600);view=null;
    ...
    Check(((JArray)final["items"]).Count==1&&final["nextCursor"]?.Type!=JTokenType.String,"FINAL_HISTORY_COUNT");
    Record("MATCH_1_EVENT_SEQUENCE_REGRESSION=NO\nMATCH_1_REPLAY_SEQUENCE_REGRESSION=NO");Record("HISTORY_COUNT_AFTER_MATCH_1=1\nEXTRA_MATCHES_STARTED=0\nMATCH_1_API_GATE=PASS\nCONSOLE_ERRORS="+errors);
    ...
    Check(Field<System.Collections.Generic.List<JObject>>(hv,"history").Count==1,"HISTORY_UI_COUNT");
    ...
    var open=hv.GetComponentsInChildren<Button>().First(x=>x.name=="View match");open.onClick.Invoke();
    ...
    Record("UNITY_MATCH_1_REPLAY_OPEN=PASS");ScreenCapture.CaptureScreenshot(Path.Combine(Dir,"replay.png"));
    Debug.Log("[SERVER6-FIVE] Match 1 API gates PASS; no Match 2 and no API restart.");
