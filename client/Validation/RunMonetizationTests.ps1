...
$output = Join-Path $PSScriptRoot 'Generated/H6Tests'
...
<ItemGroup>
<Compile Include="../../MonetizationTests.cs" /><Compile Include="../../../DominoGame/Assets/_Domino/Scripts/Rewards/*.cs" />
...
[IO.File]::WriteAllText((Join-Path $output 'H6Tests.csproj'), $project)
dotnet run --project (Join-Path $output 'H6Tests.csproj') --configuration Release --verbosity quiet
if ($LASTEXITCODE -ne 0) { throw 'H6 client tests failed' }
