...
$output = Join-Path $PSScriptRoot 'Generated/M3Tests'
...
[IO.File]::WriteAllText((Join-Path $output 'M3Tests.csproj'), $project)
$env:DOMINO_M1_ROOT = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../..'))
dotnet run --project (Join-Path $output 'M3Tests.csproj') --configuration Release --verbosity quiet
if ($LASTEXITCODE -ne 0) { throw 'M3 client tests failed' }
