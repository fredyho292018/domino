...
$output = Join-Path $PSScriptRoot 'Generated/H5Tests'
...
<ItemGroup>
<Compile Include="../../RoundRewardTests.cs" /><Compile Include="../../../DominoGame/Assets/_Domino/Scripts/Rewards/*.cs" />
...
[IO.File]::WriteAllText((Join-Path $output 'H5Tests.csproj'), $project)
dotnet run --project (Join-Path $output 'H5Tests.csproj') --configuration Release --verbosity quiet
if ($LASTEXITCODE -ne 0) { throw 'H5 client tests failed' }
