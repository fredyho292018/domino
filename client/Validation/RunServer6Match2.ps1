...
$run=Join-Path $PSScriptRoot 'Generated/SERVER6-match2'
 ...
 for($n=2;$n -le 2;$n++){
  ...
  while(!$process.HasExited){
   ...
   if(!$matched -and $found.Count -eq 3 -and (Test-Path $matchFile)){
    ...
    & C:/Python312/python.exe (Join-Path $PSScriptRoot 'ReadServer6Match2Seats.py') initial
  ...
  & C:/Python312/python.exe (Join-Path $PSScriptRoot 'ReadServer6Match2Seats.py') final
  if($LASTEXITCODE -ne 0){throw 'FINAL_SEAT_EVIDENCE_FAILED'}
  Set-Content (Join-Path $run "opponents-done-$n") 'PASS'
  Await-File (Join-Path $run "gate-$n") 180
  Write-Output "MATCH_${n}_GATE=PASS"
 }
 Write-Output 'COORDINATOR_MATCH_2=PASS'
}catch{
