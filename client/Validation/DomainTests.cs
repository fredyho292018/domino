...
static class DomainTests
{
    ...
    static void Main()
    {
        ...
        PerspectiveTests.Run();
        // Keep the historic complete trace under its original configuration.
        // Current scoring is covered separately, including a differential gameplay check.
        string trace = RegressionTrace.Compute(() => new ClientGame(ConfigurationTests.Rules(d => { d.finishScoring.source = "ALL_OTHER_PLAYERS"; d.blockedScoring.source = "ALL_OTHER_PLAYERS"; })));
        ...
        TeamScoringTests.Run();
        Console.WriteLine("TEAM_SCORING_TRACE=" + RegressionTrace.Compute(() => new ClientGame(ConfigurationTests.Rules())));
        ScoringTests();
        CompactLayoutTests();
    ...
    {
        // Historical configurable policy fixture; the shipped 2v2 policy is tested below.
        var rules = ConfigurationTests.Rules(d=> { d.targetScore=500; d.tie.repeatedTie="MULTIPLY"; d.finishScoring.source="ALL_OTHER_PLAYERS"; d.blockedScoring.source="ALL_OTHER_PLAYERS"; });
        ...
        to200.Apply(RoundScoring.Evaluate(standard,new[]{0,40,30,50},0,false,1));
        ...
        doubled.NextRound();
        var thirty = RoundScoring.Evaluate(standard,new[]{0,12,7,8},0,false,doubled.Multiplier);
