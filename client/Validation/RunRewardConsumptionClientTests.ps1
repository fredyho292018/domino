...
$output = Join-Path $PSScriptRoot 'Generated/H4Tests'
...
<ItemGroup>
<Compile Include="../../RewardConsumptionClientTests.cs" />
...
[IO.File]::WriteAllText((Join-Path $output 'H4Tests.csproj'), $project)
dotnet run --project (Join-Path $output 'H4Tests.csproj') --configuration Release --verbosity quiet
if ($LASTEXITCODE -ne 0) { throw 'H4 client tests failed' }
