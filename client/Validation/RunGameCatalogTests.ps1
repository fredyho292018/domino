...
$output = Join-Path $PSScriptRoot 'Generated/M1Tests'
...
[IO.File]::WriteAllText((Join-Path $output 'M1Tests.csproj'), $project)
$env:DOMINO_M1_ROOT = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../..'))
dotnet run --project (Join-Path $output 'M1Tests.csproj') --configuration Release --verbosity quiet
if ($LASTEXITCODE -ne 0) { throw 'M1 client tests failed' }
